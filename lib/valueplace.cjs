...
// (занятость, суточный лимит). state: {busy, placedToday, maxPerDay, placedKeys}. Возврат {candidate}|{skip}.
function choosePlacement(sigs, cfg = {}, state = {}) {
  ...
  const cand = (sigs || []).filter((s) => eligible(s, cfg) && !placed.has(sigKey(s)))
