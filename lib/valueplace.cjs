...
function choosePlacement(sigs, cfg = {}, state = {}) {
  ...
  const maxPerEvent = cfg.maxPerEvent || 0;                     // сколько исходов максимум ставим на ОДНО событие
  const eventCount = (s) => { const pre = s.t1 + "~" + s.t2 + "|"; let n = 0; for (const k of placed) if (k.indexOf(pre) === 0) n++; return n; };
  const cand = (sigs || [])
    .filter((s) => eligible(s, cfg) && !placed.has(sigKey(s)) && !(maxPerEvent && eventCount(s) >= maxPerEvent))
