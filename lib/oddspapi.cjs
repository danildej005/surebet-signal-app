...
function outcomesByMarket(fixture, bookmaker) {
  ...
  for (const [mid, mv] of Object.entries(b.markets)) {
    // marketActive=false бывает у будущих матчей с валидными ценами — НЕ пропускаем рынок целиком,
    // фильтруем по активности отдельного исхода ниже (ov.players.0.active).
    const om = {};
