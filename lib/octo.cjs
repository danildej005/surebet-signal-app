...
// puppeteer-core 25.x — чистый ESM ("type":"module"), а main-процесс Electron — CJS на Node 20.18 (без
// require(ESM)) → обычный require падает ERR_REQUIRE_ESM. Грузим через динамический import() (умеет ESM из CJS).
async function connect(uuid, opts = {}) {
  ...
  let puppeteer;
  try { const mod = await import("puppeteer-core"); puppeteer = mod.default || mod; }
  catch (e) { return { ok: false, error: "puppeteer-core не загрузился: " + e.message }; }
  try {
