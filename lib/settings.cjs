...
const DEFAULTS = {
  ...
  valueRandomTest: false,    // ЭКСПЕРИМЕНТ: ставить СЛУЧАЙНЫЕ исходы (без фильтра value/вилки), но считать/логировать их ценность.
                             // Цель — отделить «режут за содержимое (value/CLV)» от «режут за носителя (бот/анонимка)». Всё остальное как в боевом.
