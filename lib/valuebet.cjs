...
// Фильтр кандидата: тип рынка в выбранных (markets пуст = все) И кэф в диапазоне [oddsMin, oddsMax] (0 = без границы).
function passFilters(meta, odds, markets = [], oddsMin = 0, oddsMax = 0) {
  if (!meta) return false;
  if (markets.length && !markets.includes(meta.marketType)) return false;
  if (oddsMin && odds < oddsMin) return false;
  if (oddsMax && odds > oddsMax) return false;
  return true;
}
...
function candidatesForFixture(pinFx, betFx, catalog, p1, p2, cfg = {}) {
  const { threshold = 0.03, method = "proportional", stake = 0, refKey = "pinnacle", softKey = "betano", markets = [], oddsMin = 0, oddsMax = 0 } = cfg;
  ...
  for (const v of findValue(pin, bet, { threshold, method })) {
    const meta = catalog.get(String(v.marketId));
    if (!passFilters(meta, v.bookOdds, markets, oddsMin, oddsMax)) continue; // фильтр маркетов/кэфа
    const d = toDesc(meta, v.outcomeId, p1, p2);
...
function candidatesVsPinnacleFair(betFx, fair, catalog, p1, p2, eventId, cfg = {}) {
  const { threshold = 0.05, stake = 0, softKey = "betano", markets = [], oddsMin = 0, oddsMax = 0 } = cfg;
  const bet = cleanMarkets(outcomesByMarket(betFx, softKey), catalog);
  const out = [];
  for (const mid of Object.keys(bet)) {
    const meta = catalog.get(String(mid));
    if (markets.length && (!meta || !markets.includes(meta.marketType))) continue; // фильтр маркетов
    for (const [oid, price] of Object.entries(bet[mid])) {
      const fp = lookupFair(fair, meta, oid);
      if (!(fp > 0) || !(price > 1)) continue;
      if (oddsMin && price < oddsMin) continue;
      if (oddsMax && price > oddsMax) continue;
      const v = price * fp - 1;
...
module.exports = { betanoBgUrl, isCleanMeta, passFilters, candidatesForFixture, scanCandidates, lookupFair, candidatesVsPinnacleFair };
