...
class ValueLiveEngine {
  ...
  // Полный пул БК с соблюдением лимита пулов (init + реинит на mismatch). УСТОЙЧИВ: GetBookmakerData отдаёт
  // rate-limit `{message, retryAfterMilliseconds}` (лимит общий на ключ) — это НЕ «пусто»: ждём сколько сказано
  // и терпеливо повторяем (иначе быстрые ретраи сами держат лимит → движок не поднимается). Пусто/сеть — до 3 раз.
  async _initBook(book) {
    const minPull = this.opts.minPullMs || 5200;
    for (let attempt = 1, rlWaits = 0; ; attempt++) {
      ...
      if (data && data.retryAfterMilliseconds != null) {         // rate-limit фида: ждём его срок (+буфер), НЕ провал
        if (++rlWaits > 12) throw new Error(book + ": затяжной rate-limit фида");
        await sleep(Math.min(Number(data.retryAfterMilliseconds) + 400, 8000));
        attempt--; continue;                                     // rate-limit не тратит попытку
      }
