...
// «Team Yandex»↔«Yandex», «LGD Gaming»↔«LGD», «Virtus.Pro»↔«Virtus.pro» (реальный кейс: Dota есть в ОБОИХ
// плечах, а пересечение было 0 — сигналы терялись). Скобочные ники «(Esports)/(Logan)» выкидываем ДО
// нормализации, потом режем шумовые токены (team/gaming/esports/fc…). Если после чистки пусто — откат к
// полному norm (устойчивость). Риск ложной склейки мал: ключ = ПАРА команд (sort), совпасть должны обе.
const TEAM_NOISE = /\b(team|esports?|e-sports|gaming|club|fc|cf|sc|ac|afc|cfc|ssc|bk|bc|cd|ud|if|fk|f\.c\.|the)\b/gi;
function teamKey(s) {
  const noParen = String(s || "").replace(/\([^)]*\)/g, " ");
  const cleaned = noParen.replace(TEAM_NOISE, " ").normalize("NFD").replace(/[̀-ͯ]/g, "").toLowerCase().replace(/[^a-z0-9]/g, "");
  return cleaned || norm(s);
}
...
function eventSync(gb, gp) {
  const flip = teamKey(gb.team1NameEn) !== teamKey(gp.team1NameEn); // teamKey: «Team Yandex»==«Yandex» → flip не врёт при разных написаниях
...
module.exports = {
  getBookmakerData, norm, teamKey, eventSync, SPORTS, sportName, gamesByPair, matchEvents, marketsByGame,
