...
function pinnacleOutcomes(markets) {
  const out = {};
  for (const x of markets || []) {
    ...
    let kind, ab;
    if (bet === "MONEYLINE") { kind = "ML"; ab = slot === "TEAM1" ? "A" : slot === "TEAM2" ? "B" : null; }
    else if (bet === "TOTAL_POINTS") { kind = "TOTAL"; ab = side === "OVER" ? "A" : side === "UNDER" ? "B" : null; }
    else if (bet === "SPREAD") { kind = "SPREAD"; ab = slot === "TEAM1" ? "A" : slot === "TEAM2" ? "B" : null; }
    else continue;
...
function valueForEvent(betanoMarkets, pinnacleMarkets, t1, t2, opts = {}) {
  ...
  const B = betanoOutcomes(betanoMarkets, t1, t2), P = pinnacleOutcomes(pinnacleMarkets);
  const sigs = [];
...
function scanValue(betanoData, pinnacleData, opts = {}) {
  ...
  for (const e of events) {
    ...
    const sigs = valueForEvent(bmg.get(e.b.textId) || [], pmg.get(e.p.textId) || [], e.b.team1NameEn, e.b.team2NameEn, opts);
    for (const s of sigs) { s.league = e.b.leagueName; s.link = e.b.link; s.score = e.b.currentScore; out.push(s); }
  }
...
function applySnapshot(state, snap) {
  ...
  for (const it of snap.marketsAdded || []) if (it.marketModel) state.markets[it.textId] = it.marketModel;
  for (const it of snap.marketsUpdated || []) if (it.marketModel) state.markets[it.textId] = it.marketModel;
  for (const it of snap.gamesRemoved || []) delete state.games[it.textId];
  for (const it of snap.gamesAdded || []) if (it.gameModel) state.games[it.textId] = it.gameModel;
...
// PURE: value-скан из ДВУХ состояний {games, markets} (синхронный путь снимков).
...
module.exports = {
  getBookmakerData, norm, gamesByPair, matchEvents, marketsByGame,
