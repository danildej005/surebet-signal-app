...
async function sendTelegram(token, chatId, text, { timeoutMs = 15000, apiBase = "https://api.telegram.org" } = {}) {
  if (!token || !chatId) return { ok: false, error: "не заданы токен/chat_id" };
  const base = String(apiBase || "https://api.telegram.org").trim().replace(/\/+$/, "");
