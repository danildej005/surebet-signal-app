...
// Реальная цена ставки из текста кнопки постановки Betano («BET NOW 2,00 € Potential winnings 5,64 €»):
// Potential winnings ÷ ставку = кэф, по которому ставка РЕАЛЬНО встанет. Кнопка исхода могла показывать
// другой (более высокий) кэф до открытия купона — гейтить надо именно эту цену. null, если не прочитать.
function slipOddsFromBtn(btnText, stake) {
  const m = /potential winnings\s*([\d.,]+)/i.exec(String(btnText == null ? "" : btnText));
  const w = m ? parseMoney(m[1]) : null;
  const s = Number(stake);
  return (w && w > 0 && s > 0) ? w / s : null;
}

// Сколько ВЫБОРОВ в тексте купона Betano (защита от экспресса: >1 → не кликаем). Кэфы пишутся с ДВУМЯ
// знаками через ТОЧКУ (2.10), линии — с одним (22.5), поэтому «X.YZ» = выбор. ВАЛЮТО-АГНОСТИК: сперва вырезаем
// денежную сводку «Bet <сумма> … Potential winnings <W>» и кнопку «BET NOW …» (суммы там НЕ выборы), затем режем
// на первом валютном символе — чтобы на не-евро домене (lat.betano.com: $2.00, песо $2.000) сумма не сошла за
// лишний выбор. Допущение: на /en/ кэфы даются ТОЧКОЙ (подтверждено .bg); имя с «Bet…» (Real Betis) не режется — \bBet требует границы слова.
function countSlipSelections(slipText) {
  const head = String(slipText == null ? "" : slipText)
    .replace(SLIP_BET_RE, " ")
    .split(/[€$£₺₴₽¥]/)[0] || "";
  return (head.match(/\b\d+\.\d\d\b/g) || []).length;
}

// Строки ПРИНЯТЫХ ставок купона Betano из innerText: «Bet <сумма> … Potential winnings <W>». Возвращает
// ["<сумма>|<W>", …]; W/сумма = реальный принятый кэф. ВАЛЮТО-АГНОСТИК (символ валюты НЕ фиксируем: €/$/песо).
// 🔴 КРИТИЧНО (баг 14.07 lat): между «Bet» и суммой БУКВЫ ЗАПРЕЩЕНЫ. Иначе кнопка постановки
// «BET NOW $1.500,00 Potential winnings $5.025,00» (на lat она несёт сумму, в отличие от голой «BET NOW» на .bg)
// сматчивается как ПРИНЯТАЯ ставка → ложное подтверждение (в аккаунте пусто, а ТГ пишет «поставлено»).
// «Bet 2,00 €» / «Bet $2.00» / «Bet $1.500,00» — проходят (в зазоре только пробел/символ валюты, без букв).
const SLIP_BET_RE = /Bet[^\dA-Za-z]{0,4}([\d.,]+)[^\d]*?Potential winnings\s*[^\d]{0,6}([\d.,]+)/gi;
function slipBetsFromText(text) {
  const out = [];
  const re = new RegExp(SLIP_BET_RE.source, "gi"); // свежий lastIndex на каждый вызов
  let m; while ((m = re.exec(String(text == null ? "" : text)))) out.push(m[1] + "|" + m[2]);
  return out;
}
...
module.exports = { vilkaStakes, round2, parseMoney, slipOddsFromBtn, countSlipSelections, slipBetsFromText };
