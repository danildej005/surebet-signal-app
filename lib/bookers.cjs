...
function pickOutcome({ desc = "", expectedOdds = 0, outcomeId = "", buttons = [], eventUrl = "", unit = null, subject = "" } = {}) {
  ...
  const poolHasNames = pool.some((b) => b.team || /[a-zA-Z]{2,}/.test(nameOf(b.text)));
  if ((c.kind === "win" || c.kind === "hcap" || c.kind === "ehcap") && c.side && !poolHasScore && poolHasNames) {
    ...
    if (targets.length) {
      const named = pool.filter((b) => { const nm = slugify(b.team || nameOf(b.text)); return nm && targets.some((t) => nm.includes(t) || t.includes(nm)); });
