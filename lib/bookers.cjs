...
function pickOutcome({ desc = "", expectedOdds = 0, outcomeId = "", buttons = [], eventUrl = "", unit = null, subject = "" } = {}) {
  ...
  // Кэф-страховка: всегда когда сторона не подтверждена именем; а для ehcap — ВСЕГДА (знак/вариант иннингов
  // мы выбрали по кэфу, поэтому обязаны проверить, что он действительно близок — иначе не тот рынок).
  if (expectedOdds && (!nameConfirmed || c.nodraw || c.kind === "ehcap")) {
