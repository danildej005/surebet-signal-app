...
function parseSurebetNav(url) {
  try {
    ...
    return {
      ...
      desc: prong.tr_terse || "",                                  // тип ставки (Ф1(+1.5), Тм(3.5)…)
    };
  } catch { return null; }
}
...
function resolveSurebetNav(url, bookers) {
  ...
  return {
    ...
    outcomeId: nav.outcomeId, expectedOdds: nav.expectedOdds, desc: nav.desc,
  };
}

// Выбрать кнопку исхода Pinnacle на странице события.
// Бренд ps3838 НЕ присылает id исхода и глубокую ссылку — только описание (tr_terse: «Ф1(+1.5)»,
// «Тб(2.5)», «П1») и кэф. Поэтому: 1) если id из вилки есть — матч по «хвосту» id (после события);
// 2) иначе — по описанию (фора/тотал) + ближайшему кэфу. buttons: [{id, text}] со страницы.
function pickPinnacleOutcome({ desc = "", expectedOdds = 0, outcomeId = "", buttons = [] } = {}) {
  const norm = (s) => String(s || "").replace(/\s+/g, " ").trim();
  const oddsOf = (t) => { const ns = norm(t).match(/\d+\.\d+/g); return ns ? Number(ns[ns.length - 1]) : null; };
  // Кнопки исходов Pinnacle: id с «|» и кэф в тексте (отсекаем навигацию ALL/MATCH/…).
  const cands = buttons.filter((b) => String(b.id || "").includes("|") && /\d+\.\d+/.test(b.text || ""));

  // 1) точный путь — по «хвосту» id из вилки (бренды, где pinnacleBrExternalId есть)
  const oid = String(outcomeId || "");
  if (oid.includes("|")) {
    const suffix = oid.split("|").slice(1).join("|");
    const hit = cands.find((b) => b.id.split("|").slice(1).join("|") === suffix);
    if (hit) return { id: hit.id, text: norm(hit.text), odds: oddsOf(hit.text), how: "id" };
  }

  // 2) по описанию вилки + ближайшему кэфу
  const d = norm(desc);
  const m = d.match(/\(\s*([+\-−]?\d+(?:[.,]\d+)?)\s*\)/); // значение в скобках: фора/тотал
  const line = m ? m[1].replace(/−/g, "-").replace(",", ".") : null;
  const over = /(^|\s)Т\s*[бБ]/.test(d) || /\bover\b/i.test(d);   // Тб / Over
  const under = /(^|\s)Т\s*[мМ]/.test(d) || /\bunder\b/i.test(d); // Тм / Under
  let pool = cands;
  if (over || under) pool = pool.filter((b) => (over ? /over/i.test(b.text) : /under/i.test(b.text)) && (!line || norm(b.text).includes(line)));
  else if (line) pool = pool.filter((b) => norm(b.text).includes(line)); // фора: текст содержит «+1.5»/«-1.5»
  ...
  if (!best) return null;
  return { id: best.id, text: norm(best.text), odds: oddsOf(best.text), how: "desc" };
}

module.exports = {
  defaultBookers, emptyProxy, buildProxyString, randomFingerprint, buildFingerprintScript, bookerForUrl,
  parseSurebetNav, resolveSurebetNav, bkToBookerId, pickPinnacleOutcome, BK_TO_BOOKER, BOOKER_KEYWORDS, UA_PRESETS, WEBGL_PRESETS,
};
