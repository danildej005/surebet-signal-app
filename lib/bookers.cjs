...
// Выбрать кнопку исхода на странице события (обе конторы).
// Вилка часто НЕ даёт id исхода (Pinnacle/ps3838 и Betano) — только описание (tr_terse:
// «Ф1(+1.5)», «Тб(2.5)», «П1») и кэф. Поэтому: 1) если id из вилки есть и совпал «хвост» — по id;
// 2) иначе — по описанию (фора/тотал/победа) + ближайшему кэфу.
// buttons: [{ i, id, text }] — i = позиция кнопки в выборке селектора (для клика по индексу).
function pickOutcome({ desc = "", expectedOdds = 0, outcomeId = "", buttons = [] } = {}) {
  ...
  const over = /Тб|больш/i.test(d) || /\bover\b/i.test(d);   // Тб / Over / больше
  const under = /Тм|меньш/i.test(d) || /\bunder\b/i.test(d); // Тм / Under / меньше

  let pool;
  if (over || under) {
    pool = cands.filter((b) => (over ? /over|больш/i.test(b.text) : /under|меньш/i.test(b.text)) && (!line || norm(b.text).includes(line)));
  } else if (line) {
    pool = cands.filter((b) => norm(b.text).includes(line)); // фора: текст содержит «+1.5»/«-1.5»
  } else {
    // победа/прочее: отсечь тоталы/форы/счёт, иначе кэф может совпасть с чужим рынком
    pool = cands.filter((b) => !/over|under|больш|меньш/i.test(b.text) && !/[+\-−]\s*\d/.test(b.text) && !/\d+\s*[-:]\s*\d+/.test(b.text));
  ...
  return ret(best, "desc");
}

module.exports = {
  defaultBookers, emptyProxy, buildProxyString, randomFingerprint, buildFingerprintScript, bookerForUrl,
  parseSurebetNav, resolveSurebetNav, bkToBookerId, pickOutcome, BK_TO_BOOKER, BOOKER_KEYWORDS, UA_PRESETS, WEBGL_PRESETS,
};
