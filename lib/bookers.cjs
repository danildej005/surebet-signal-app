...
function pickOutcome({ desc = "", expectedOdds = 0, outcomeId = "", buttons = [], eventUrl = "", unit = null, subject = "" } = {}) {
  ...
  if ((c.kind === "win" || c.kind === "hcap" || c.kind === "ehcap") && c.side && poolHasNames) {
    ...
    if (targets.length) {
      ...
      // Оставляем: счёт (сторона в счёте) + безымянные (Pinnacle-фора, различит кэф) + именованные ТОЛЬКО нашей
      // стороны. ВЫкидываем именованные кнопки ЧУЖОГО игрока — это и был баг: при наличии счёта в пуле блок имени
      // отключался целиком, и чужая фора («Marvin -1.5» вместо «Calvin -1.5») проскакивала по кэфу.
      const kept = pool.filter((b) => scoreOf(b.text) || !isNamed(b) || matchesOur(b));
