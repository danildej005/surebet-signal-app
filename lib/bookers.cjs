...
const BK_TO_BOOKER = {
  pinnaclesports: "pinnacle", pinnacle888: "pinnacle", pinnacle: "pinnacle",
...
function parseSurebetNav(url) {
  try {
    ...
    const targetUrl =
      (prong.markers && prong.markers.link) ||
      ...
      null;
    return { bk, prongIndex: idx, targetUrl };
  } catch { return null; }
}
...
function resolveSurebetNav(url, bookers) {
  ...
  if (!nav) return null;
  const bookerId = BK_TO_BOOKER[nav.bk] || null;
  const booker = Array.isArray(bookers) ? bookers.find((b) => b.id === bookerId) || null : null;
  return { bk: nav.bk, booker, targetUrl: nav.targetUrl || (booker && booker.url) || null };
}

module.exports = {
  defaultBookers, randomFingerprint, buildFingerprintScript, bookerForUrl,
  parseSurebetNav, resolveSurebetNav, BK_TO_BOOKER, BOOKER_KEYWORDS, UA_PRESETS, WEBGL_PRESETS,
};
