...
// Client Hints (Sec-CH-UA + navigator.userAgentData) из строки UA. КРИТИЧНО для Cloudflare:
// если UA говорит «Chrome/131», а CH отдают Electron/другую версию — мгновенный детект бота.
// Передаётся в CDP Network.setUserAgentOverride → согласует и заголовки запросов, и navigator.userAgentData.
function uaMetadata(ua, platformVersion) {
  const s = String(ua || "");
  const m = s.match(/Chrome\/(\d+)/);
  const major = m ? m[1] : "131";
  // GREASE-бренд варьируется по версии Chrome; берём наблюдаемые значения, дефолт — общий.
  const greaseV = ({ "129": "8", "130": "99", "131": "24", "132": "24", "133": "99" })[major] || "24";
  const brands = [
    { brand: "Chromium", version: major },
    { brand: "Google Chrome", version: major },
    { brand: "Not_A Brand", version: greaseV },
  ];
  const full = major + ".0.0.0";
  const fullVersionList = [
    { brand: "Chromium", version: full },
    { brand: "Google Chrome", version: full },
    { brand: "Not_A Brand", version: greaseV + ".0.0.0" },
  ];
  return {
    brands,
    fullVersionList,
    fullVersion: full,
    platform: "Windows",
    platformVersion: platformVersion || "10.0.0",
    architecture: "x86",
    bitness: "64",
    model: "",
    mobile: false,
...
function buildFingerprintScript(fp) {
  ...
  // window.chrome — на обычной странице Chrome объект есть; в Electron бывает неполным. Добавляем недостающее.
  try {
  ...
  // navigator.plugins / mimeTypes как у обычного Chrome (встроенный PDF). Только если пусто — не ломаем родной PluginArray.
  try {
    if (!navigator.plugins || navigator.plugins.length === 0) {
      const pdfMimes = [ { type: 'application/pdf', suffixes: 'pdf', description: 'Portable Document Format' }, { type: 'text/pdf', suffixes: 'pdf', description: 'Portable Document Format' } ];
      const names = ['PDF Viewer', 'Chrome PDF Viewer', 'Chromium PDF Viewer', 'Microsoft Edge PDF Viewer', 'WebKit built-in PDF'];
      const plugins = names.map((name) => { const p = { name, filename: 'internal-pdf-viewer', description: 'Portable Document Format', length: pdfMimes.length, item: (i) => pdfMimes[i], namedItem: (t) => pdfMimes.find((m) => m.type === t) || null }; pdfMimes.forEach((m, i) => { p[i] = m; }); return p; });
      plugins.item = (i) => plugins[i]; plugins.namedItem = (n) => plugins.find((p) => p.name === n) || null; plugins.refresh = () => {};
      def(navigator, 'plugins', plugins);
      const mimes = pdfMimes.slice(); mimes.item = (i) => mimes[i]; mimes.namedItem = (t) => mimes.find((m) => m.type === t) || null;
      def(navigator, 'mimeTypes', mimes);
    }
  } catch(e){}
  // permissions.query: notifications должен совпадать с Notification.permission (классический анти-headless тест).
  try {
    const q = navigator.permissions && navigator.permissions.query;
    if (q) navigator.permissions.query = function (p) { return (p && p.name === 'notifications') ? Promise.resolve({ state: (window.Notification && Notification.permission) || 'default', onchange: null }) : q.apply(this, arguments); };
  } catch(e){}
})();`;
}
...
module.exports = {
  defaultBookers, emptyProxy, buildProxyString, randomFingerprint, randomUA, buildFingerprintScript, uaMetadata, bookerForUrl,
