...
function pickOutcome({ desc = "", expectedOdds = 0, outcomeId = "", buttons = [], eventUrl = "", unit = null, subject = "" } = {}) {
  ...
  if (c.kind === "total") {
    ...
    pool = cands.filter((b) => (c.over ? /over|больш/i.test(b.text) : /under|меньш/i.test(b.text)) && totalLine(b.text) === want && !isCompound(b.text));
  } else if (c.kind === "hcap") {
    const want = numOf(c.line);
    ...
    pool = cands.filter((b) => !/over|under|больш|меньш/i.test(b.text) && hcapLine(b.text) === null && scoreOf(b.text) === null && !isCompound(b.text) && !/\d\s*\+/.test(norm(b.text)));
  }
  ...
  if (c.kind === "hcap" && c.side && subject && !nameConfirmed && !poolHasScore && !poolHasNames) {
    const want = numOf(c.line);
    const ml = cands.filter((b) => !/over|under|больш|меньш/i.test(b.text) && hcapLine(b.text) === null && scoreOf(b.text) === null && !isCompound(b.text) && !/\bto win to\b/i.test(norm(b.text)) && /[a-z]{2,}/i.test(nameOf(b.text)));
