...
function classifyDesc(desc) {
  ...
  m = d.match(/^([12])\s+1-2$/);
  if (m) return { kind: "win", side: m[1], nodraw: true };
...
function pickOutcome({ desc = "", expectedOdds = 0, outcomeId = "", buttons = [], eventUrl = "", unit = null, subject = "" } = {}) {
  ...
  if (c.kind === "total") {
    ...
    // Защита nodraw («1-2»): если на странице есть ничья — это 3-исходный рынок (реальный DNB),
    // победителем подменять НЕЛЬЗЯ (кэфы разные → не настоящий хедж). Берём «1-2» только в 2-исходных видах.
    if (c.nodraw && cands.some((b) => /\b(draw|empate|ничья|tie)\b/i.test(b.text) || /^x\s+\d/i.test(norm(b.text)))) return null;
  ...
  if (expectedOdds && (!nameConfirmed || c.kind === "ehcap")) {
