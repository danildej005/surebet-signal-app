...
function pickOutcome({ desc = "", expectedOdds = 0, outcomeId = "", buttons = [], eventUrl = "", unit = null, subject = "" } = {}) {
  ...
  if (c.kind === "hcap" && c.side && subject && !nameConfirmed && !poolHasScore && !poolHasNames) {
    ...
    const ourBtn = ml.find((b) => { const nm = slugify(nameOf(b.text)); return nm && (nm.includes(subj) || subj.includes(nm)); });
    const ourO = ourBtn ? oddsOf(ourBtn.text) : null;
    const oppOdds = ml.filter((b) => b !== ourBtn).map((b) => oddsOf(b.text)).filter((x) => x != null);
    const opp = oppOdds.length ? Math.min(...oppOdds) : null;
