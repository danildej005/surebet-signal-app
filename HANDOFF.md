...
Текущая версия: **0.7.14**. Релизы собираются вручную с Mac, публикуются в GitHub Releases.
...
## Оркестратор «Запуск бота» (готово в 0.7.14, dry-run)
- `runBot(live)` в main.cjs: `pickVilka()` (читает `tbody.surebet_record` в окне surebet, ищет пару Betano + Pinnacle **Delayed** по `a[href*="/nav/surebet/prong/"]`, `bk!=ps3838`) → `routeLeg()` обоих плеч (резолв→окно→дойти до события) → `selectLegOutcome()` (выбор+макс) обоих → `vilkaStakes()` → `fillStakeOnly()` обоих → сводка. live только при тумблере БОЕВОЙ → `clickPlace()` обоих.
- `placeBet` разбит на шаги: `selectLegOutcome` / `fillStakeOnly` / `clickPlace` (переиспользуются оркестратором). IPC `run-bot`. Клик по плечу теперь тоже через `routeLeg` (общий путь).
- Чтение макса: `readBookmakerMax` — Pinnacle текст «Max bet USDT …», Betano кнопка `.max-button`→поле. `parseMoney` (US/EU).
- Панель: поле «Лимит вилки на плечо €» (`vilkaLimitEur`), кнопка «Запуск бота», сводка `botResult`.
- **Сетовая фора −1.5 ⟺ счёт** (БО3): `pickOutcome` добавляет счёт-кандидата (Betano берёт счёт, Pinnacle — фору; разводит кэф). Сверка по имени не применяется при наличии счёт-кандидата.

## Дальше (роадмап)
1. **Проверить вживую** `pickVilka` (читаются ли href плеч из DOM сканера) + полный цикл dry-run. Если href нет в DOM — другой способ достать ссылки плеч.
2. **Боевой клик** — проверить на копеечной ставке (тумблер БОЕВОЙ).
3. Непрерывный режим (бот сам мониторит и гоняет цикл), а не по кнопке.
