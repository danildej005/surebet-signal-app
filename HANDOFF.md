...
Текущая версия: **0.7.1**. Релизы собираются вручную с Mac, публикуются в GitHub Releases.
...
2. **Кнопка «Запуск бота»** — оркестратор полного цикла: вилка → оба плеча → события → купон → сумма → ставка (под liveMode).
3. **Расчёт сумм плеч** (балансировка арбитража) — пока сумма вводится вручную.
...
- **`resolveEventViaNav` (surebet-проход) — ТОЛЬКО когда у конторы нет глубокой ссылки** (Betano = только домен). Для **Pinnacle НЕ трогать surebet** (глубокая ссылка уже в `markers.link`), иначе лишний проход цепляет **логин surebet** при клике по плечу. Условие: запускать проход, если `new URL(initial).pathname` пустой/«/». (Этот симптом «логин surebet» возвращался дважды от РАЗНЫХ причин — см. 0.4.0 и 0.7.0.)
